...
normDataset <- function(dt){
  parmNormedList <- lapply(grep("_CP_|_QI_|_PA_|SpotCellCount|Lineage",colnames(dt),value = TRUE), function(parm){
    dt <- dt[,paste0(parm,"_MedNorm") := normWellsWithinPlate(.SD, value=parm, baseECM = ".*",baseL = "FBS"), by="Barcode"]
    return(dt)
  })
...
calc2NProportion <- function(x){
  ...
  if(!is.numeric(proportion2N))stop(paste("Calculating proption error on ", x))
  return(proportion2N)
}
